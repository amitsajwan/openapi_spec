...
def _set_value_by_path(data_dict: Dict[str, Any], path: str, value: Any):
    ...
    if keys: 
        ...
        logger.warning(f"Path was empty for _set_value_by_path. Original dict might be directly modified if it was the target.")
...
class ExecutionGraphDefinition:
    ...
    def __init__(self, graph_execution_plan: GraphOutput, api_executor: APIExecutor):
        ...
        self.runner: CompiledGraph = self._build_and_compile_graph()
    ...
    def _apply_input_mappings(
        ...
        for mapping in node_definition.input_mappings:
            ...
            logger.debug(f"Node '{node_definition.effective_id}': InputMapping - Found value for source path '{mapping.source_data_path}'. Target: '{mapping.target_parameter_name}' in '{mapping.target_parameter_in}'.")
            ...
            if mapping.target_parameter_in == "path":
                ...
                pass # Handled in _prepare_api_request_components
    ...
    def _prepare_api_request_components(
        ...
        if not isinstance(body_after_placeholder_resolution, dict) and body_for_field_mappings:
            ...
            final_body_payload = body_for_field_mappings # This is the dict with field mappings applied
    ...
    async def _handle_node_confirmation_interrupt_if_needed(
        ...
        if node_definition.requires_confirmation and not is_already_confirmed:
            logger.info(f"Node '{operationId}' requires confirmation (Path: {api_path}).")
            interrupt_data_for_ui = {
                "type": "api_call_confirmation", "operationId": operationId,
                "method": node_definition.method, "path": api_path,
                "query_params_to_confirm": query_params, "payload_to_confirm": body_payload,
    ...
    def _apply_confirmed_data_to_request(
        ...
        if current_confirmed_data.get(confirmation_key):
            ...
            if "modified_payload" in confirmed_details:
                ...
                logger.info(f"Node '{operationId}': Applied modified payload from confirmation.")
    ...
    def _make_node_runnable(
        ...
        async def node_executor(state: ExecutionGraphState) -> Dict[str, Any]:
            ...
            try:
                ...
                await self._handle_node_confirmation_interrupt_if_needed(node_definition, state, api_path, query_params, body_payload, headers)
                final_body, final_params, final_headers = self._apply_confirmed_data_to_request(node_definition, state, body_payload, query_params, headers)
                api_call_result, extracted_data = await self._execute_api_and_process_outputs(node_definition, api_path, final_params, final_body, final_headers)
                
                logger.info(f"--- [Graph 2] Node End: {effective_id} ---")
                return {"api_results": {effective_id: api_call_result}, "extracted_ids": extracted_data, "error": None}
            except Interrupt: 
                logger.info(f"Node '{effective_id}' raising Interrupt for confirmation.")
                raise
            except Exception as e:
                error_message = f"Error in node {effective_id}: {type(e).__name__} - {e}"
                logger.error(error_message, exc_info=True)
                return {"error": error_message, "api_results": {effective_id: {"error": error_message, "status_code": "NODE_EXCEPTION"}}}
    ...
    def _build_and_compile_graph(self) -> CompiledGraph:
        ...
        for node_def in self.graph_plan.nodes:
            if node_def.effective_id.upper() in ["START_NODE", "END_NODE"]: continue 
            if not node_def.method or not node_def.path:
                raise ValueError(f"API Node '{node_def.effective_id}' in plan must have 'method' and 'path'.")
        ...
        for edge in self.graph_plan.edges:
            source_is_langgraph_start = edge.from_node.upper() == "START"
            target_is_langgraph_end = edge.to_node.upper() == "END"

            if not source_is_langgraph_start and edge.from_node not in node_ids_in_plan: 
                logger.warning(f"Edge source '{edge.from_node}' not in plan nodes. Skipping.")
                continue
            if not target_is_langgraph_end and edge.to_node not in node_ids_in_plan:
                logger.warning(f"Edge target '{edge.to_node}' not in plan nodes. Skipping.")
                continue

            if source_is_langgraph_start:
                builder.add_edge(START, edge.to_node)
                has_start_edge = True
        ...
        if not actual_api_nodes:
             raise ValueError("Execution plan contains no executable API nodes.")

        if not has_start_edge:
            builder.set_entry_point(actual_api_nodes[0].effective_id)
            logger.info(f"No explicit START edge. Set entry point to: '{actual_api_nodes[0].effective_id}'.")
    ...
    def get_runnable_graph(self) -> CompiledGraph:
        return self.runner
