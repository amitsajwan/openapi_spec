...
from models import BotState, GraphOutput, Node, InputMapping, OutputMapping # Added OutputMapping
from utils import (
...
# --- Configurable Limits ---
MAX_PAYLOAD_SAMPLES_TO_GENERATE_INITIAL = int(os.getenv("MAX_PAYLOAD_SAMPLES_TO_GENERATE_INITIAL", "3")) # Increased slightly
...
MAX_APIS_IN_PROMPT_SUMMARY_LONG = int(os.getenv("MAX_APIS_IN_PROMPT_SUMMARY_LONG", "20")) # Increased for more context
...
class OpenAPICoreLogic:
        ...
        for path_url, path_item in paths.items():
            ...
            for method, operation_details in path_item.items():
                ...
                api_info = {
                    ...
                    'path': path_url, 'method': method.upper(), 
    ...
    def _generate_payload_descriptions(self, state: BotState, target_apis: Optional[List[str]] = None, context_override: Optional[str] = None):
        ...
        if target_apis: 
            ...
            apis_with_payload_info = [
                ...
                if api.get('requestBody') or \
                   any(p.get('in') in ['body', 'formData'] for p in api.get('parameters', [])) 
            ]
        ...
        for api_op in apis_to_process:
            ...
            for p_idx, p_detail in enumerate(api_op.get('parameters', [])):
                if p_idx >= 5: # Limit to first 5 params for prompt brevity
            ...
            if api_op.get('requestBody') and isinstance(api_op['requestBody'], dict):
                ...
                if schema:
                    request_body_schema_str = json.dumps(schema, indent=2)[:500] + "..." # Truncate schema
            ...
            prompt = (
                ...
                f"Request Body Schema (if application/json): {request_body_schema_str}\n"
                f"Successful (2xx) Response Schema (sample, if application/json): {success_response_schema_str}\n\n"
                ...
                f"Use plausible, real-world example values based on the parameter names, types, and API summary. For example, if a field is 'email', use 'user@example.com'. If 'count', use a number like 5. "
                f"Also, provide a brief description of the expected JSON response structure for a successful call. "
    ...
    def _generate_execution_graph(self, state: BotState, goal: Optional[str] = None) -> BotState:
        ...
        for idx, api in enumerate(state.identified_apis):
            ...
            # Include parameter info in summary for better context
            params_str_parts = []
            if api.get('parameters'):
                for p_idx, p_detail in enumerate(api['parameters']):
                    if p_idx >= 2: # Max 2 params in this summary line
                        params_str_parts.append("...")
                        break
                    params_str_parts.append(f"{p_detail.get('name')}({p_detail.get('in')})")
            ...
            api_summaries_for_prompt.append(
                ...
                f"summary: {api.get('summary', 'N/A')[:100]}, {params_str}, " 
        ...
        # Enhanced prompt for graph generation
        prompt = f"""
        ...
        - To get details of a specific item, you might first need to list items or create an item.
        - A 'create' operation (POST) should usually precede 'get by ID' (GET), 'update' (PUT/PATCH), or 'delete' (DELETE) operations for that specific resource.
        - If the goal is "create a user and then retrieve their details", the graph must include the 'create user' API call BEFORE the 'get user by ID' call.
        - Data created in one step (e.g., an ID from a POST response) should be mapped to subsequent steps that need it.

        The graph must adhere to the Pydantic models:
        InputMapping: {{"source_operation_id": "str_effective_id_of_source_node", "source_data_path": "str_jsonpath (e.g., '$.response_field_name', '$.items[0].id')", "target_parameter_name": "str_param_name_in_target_node", "target_parameter_in": "Literal['path', 'query', 'body', 'body.fieldName']"}}
        OutputMapping: {{"source_data_path": "str_jsonpath (e.g., '$.id', '$.data.token')", "target_data_key": "str_key_for_shared_data_pool (e.g., 'createdUserId', 'authToken')"}}
        Node: {{"operationId": "str_from_api_list", "display_name": "optional_unique_str_if_opId_reused", "method": "str (GET/POST etc)", "path": "str_from_api_list (/users/{{{{userId}}}})", "summary": "str_from_api_list", "description": "str (purpose in this workflow)", "input_mappings": [InputMapping], "output_mappings": [OutputMapping], "requires_confirmation": "bool (true for POST/PUT/DELETE usually)", "payload": {{ "template_key": "realistic_example_value or {{{{placeholder_from_output_mapping}}}}" }} }}
        Edge: {{"from_node": "str_effective_id_of_source_node_or_START", "to_node": "str_effective_id_of_target_node_or_END", "description": "optional_str_reason_for_edge"}}
        GraphOutput: {{"graph_id": "optional_str", "nodes": [Node], "edges": [Edge], "description": "str (overall workflow)", "refinement_summary": "str (e.g., 'Initial graph with X, Y, Z steps following create-then-get pattern.')"}}

        Key Instructions:
        1.  Create "START_NODE" (`operationId: "START_NODE"`, `method: "SYSTEM"`, `path: "/start"`) and "END_NODE" (`operationId: "END_NODE"`, `method: "SYSTEM"`, `path: "/end"`). These system nodes should not have input/output mappings, payload, or require confirmation.
        ...
        3.  For `payload` in POST/PUT/PATCH nodes:
            - Use realistic example values for fields (e.g., for "name": "Example User", for "email": "user@example.com").
            - If a field's value should come from a previous step's output, use a placeholder like `{{{{some_target_data_key_from_output_mapping}}}}`. Ensure this placeholder matches a `target_data_key` from an `OutputMapping` of a preceding node.
        4.  Define `input_mappings` for each API node if it depends on data from a previous node's output.
            - `source_operation_id` should be the `effective_id` (operationId or display_name) of the node that produces the data.
            - `source_data_path` should be a JSONPath expression to extract data from the source node's typical JSON response (e.g., '$.id', '$.data.items[0].name').
            - `target_parameter_name` is the name of the parameter in the current node (e.g., 'userId' for a path parameter, or a field name for a body parameter if `target_parameter_in` is 'body.fieldName').
            - `target_parameter_in` specifies where the data goes ('path', 'query', 'header', 'body' for full body replacement, or 'body.fieldName' for a specific field in a JSON body).
        5.  Define `output_mappings` for each API node, specifying how to extract key data from its response into a shared data pool using `target_data_key`. This key will be used by subsequent nodes' `input_mappings` (via placeholders in payload) or `source_data_path`.
        6.  Set `requires_confirmation: true` for nodes that modify data (POST, PUT, DELETE, PATCH), `false` otherwise.
        7.  Connect nodes with `edges`. START_NODE must connect to the first API operation(s). Last API operation(s) must connect to END_NODE. Ensure all `from_node` and `to_node` in edges match an `effective_id` of a node in the `nodes` list or are "START"/"END".
        8.  Ensure the sequence of operations is logical (e.g., create before get/update/delete).
        9.  Provide an overall `description` of the workflow and a `refinement_summary`.
        ...
        {{
            ...
            "summary": "Create a new user", "description": "Creates a new user in the system.",
            "payload": {{ "name": "John Doe", "email": "john.doe@example.com", "age": 30 }},
            "input_mappings": [],
            "output_mappings": [
                {{"source_data_path": "$.id", "target_data_key": "newlyCreatedUserId"}},
                {{"source_data_path": "$.status", "target_data_key": "userCreationStatus"}}
            ],
            "requires_confirmation": true
        }}
        {{
            "operationId": "getUserDetails", "method": "GET", "path": "/users/{{{{newlyCreatedUserId}}}}",
            "summary": "Get details for a specific user", "description": "Retrieves user profile using the ID from the creation step.",
            "payload": null,
            "input_mappings": [
                {{"source_operation_id": "createUser", "source_data_path": "$.newlyCreatedUserId", "target_parameter_name": "newlyCreatedUserId", "target_parameter_in": "path"}}
            ],
            "output_mappings": [
                {{"source_data_path": "$.email", "target_data_key": "userDetailsEmail"}}
            ],
    ...
    def verify_graph(self, state: BotState) -> BotState:
        ...
        try:
            ...
            if "START_NODE" in node_ids:
                ...
                if not start_outgoing and len(state.execution_graph.nodes) > 2 : 
            ...
            issues.append(f"Graph structure is invalid: {str(ve)[:200]}...") 
        except Exception as e: 
    ...
    def refine_api_graph(self, state: BotState) -> BotState:
        ...
        1.  Strictly adheres to the Pydantic model structure for GraphOutput, Node, Edge, InputMapping, OutputMapping (including 'payload' field for Nodes with realistic examples or placeholders like `{{{{key_from_output_mapping}}}}` if data comes from a prior step).
        ...
        7.  Addresses the specific feedback. If feedback mentions adding/removing/reordering specific operations, reflect that. Ensure logical dependencies (e.g., create before get/update).
    ...
    def interactive_query_executor(self, state: BotState) -> BotState:
        ...
        try:
            if action_name == "rerun_payload_generation":
                ...
                self.setup_workflow_execution(state) 
